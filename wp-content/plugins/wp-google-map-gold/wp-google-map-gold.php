...
if ( ! class_exists( 'Google_Maps_Pro' ) ) {
	...
	class Google_Maps_Pro
	{
		...
		function _init() {
			...
			add_action( 'admin_enqueue_scripts', array( $this, 'wpgmp_frontend_scripts' ) );
   ...
   <input type="hidden" name="wpgmp_hidden_flag" value="true" />
	...
	<?php
		...
		function wpgmp_frontend_scripts() {
			...
			'wpgmp-backend'  => WPGMP_CSS.'backend.css',
			);
	...
	<?php
		...
		private function _load_files() {
			...
			'class.tabular.php',
			'class.template.php',
			'class.frontend-tabular.php',
			'class.frontend-templates.php',
