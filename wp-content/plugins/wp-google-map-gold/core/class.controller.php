	...
	class WPGMP_Core_Controller {
		 ...
		 */
	     function __construct($objectType) {
			...
			} else if ( file_exists( WPGMP_MODEL.$this->entity.'/model.'.$this->entity.'.php' ) ) {
		...
		public function display($view, $options = array()) {
			...
			if ( ! empty( $view ) ) {
				if(file_exists(WPGMP_MODEL. "{$this->entity}/views/".$view)){
					return include( WPGMP_MODEL. "{$this->entity}/views/".$view );
				}
				else if(is_object($this->entityObj)){
					return $this->entityObj->views($view);
				}
			}
		}
		/**
