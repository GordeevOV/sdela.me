...
<form action="<?php echo $search_url; ?>" class="w2dc-content w2dc-search-map-form w2dc-search-form">
	...
	<?php if (get_option('w2dc_show_where_search')): ?>
		<div class="w2dc-row">
			...
			<?php else: ?>
			<div class="w2dc-col-md-<?php echo $col_md; ?>">
				<input type="text" name="address" id="address_<?php echo $random_id; ?>" class="w2dc-form-control" placeholder="<?php esc_attr_e('Enter address or zip code', 'W2DC'); ?>" value="<?php if (isset($_GET['address'])) echo esc_attr(stripslashes($_GET['address'])); ?>" />
			</div>
			<?php endif; ?>
			<?php endif; ?>
