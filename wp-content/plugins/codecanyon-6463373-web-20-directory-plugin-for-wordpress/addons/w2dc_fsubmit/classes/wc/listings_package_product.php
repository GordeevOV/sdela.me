...
class w2dc_listings_package_product {
	...
	public function set_listings_numbers($status, $order_id) {
		...
		if ($status == 'completed') {
			...
			foreach ($items AS $item_id=>$item) {
				if (($product = wc_get_product($item['item_meta']['_product_id'][0])) && $product->product_type == 'listings_package') {
	...
	public function save_user_profile_fields($user_id) {
		...
		foreach ($w2dc_instance->levels->levels_array as $level) {
			update_user_meta($user_id, '_listings_unlimited_'.$level->id, wc_clean($_POST['_listings_unlimited_'.$level->id]));
			update_user_meta($user_id, '_listings_number_'.$level->id, wc_clean($_POST['_listings_number_'.$level->id]));
		}
	}
	...
	public function available_listings_descr($level_id, $action_string) {
		...
		if ($listings_number[$level_id]['unlimited'])
			$out = sprintf(__('You can %s any number of listings for <strong>free</strong>', 'W2DC'), $action_string);
		elseif ($number = $listings_number[$level_id]['number'])
			$out = sprintf(__('You can %s <strong>%d</strong> %s for <strong>free</strong>', 'W2DC'), $action_string, $number, _n('listing', 'listings', $number, 'W2DC'));
