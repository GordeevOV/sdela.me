				...
				if (!isset($_POST['listing_id']) || !isset($_POST['listing_id_hash']) || !is_numeric($_POST['listing_id']) || md5($_POST['listing_id'] . wp_salt()) != $_POST['listing_id_hash']) {
					...
					if ($errors) {
						...
						if (is_wp_error($result)) {
							...
							if (get_option('w2dc_newlisting_admin_notification') && get_option('w2dc_admin_notifications_email')) {
								...
								wp_mail(get_option('w2dc_admin_notifications_email'), $subject, $body, $headers);
							}
