...
class w2dc_content_field {
	...
	public function create($array) {
		...
		if ($this->isCategories())
			$insert_update_args['categories'] = serialize(w2dc_getValue($array, 'categories_list'));
...
add_filter('w2dc_content_field_create_edit_args', 'w2dc_filter_content_field_categories', 10, 3);
function w2dc_filter_content_field_categories($insert_update_args, $content_field, $array) {
	global $sitepress;
...
function w2dc_save_content_field($content_field_id, $content_field, $array) {
	...
	if (function_exists('wpml_object_id_filter') && $sitepress) {
		if ($sitepress->get_default_language() != ICL_LANGUAGE_CODE && $content_field->isCategories()) {
			update_option('w2dc_wpml_content_field_categories_'.$content_field_id.'_'.ICL_LANGUAGE_CODE, w2dc_getValue($array, 'categories_list'));
		}
	}
