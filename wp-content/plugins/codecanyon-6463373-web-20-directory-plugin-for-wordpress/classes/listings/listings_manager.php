...
class w2dc_listings_manager {
	...
	public function changeExpirationDate() {
		...
		} elseif ($error_string = $w2dc_form_validation->error_string())
			...
			if ($this->loadCurrentListing($listing_id)) {
				...
				if (isset($_GET['claim_action']) && ($_GET['claim_action'] == 'approve' || $_GET['claim_action'] == 'decline')) {
					if ($_GET['claim_action'] == 'approve') {
						...
						if (get_option('w2dc_claim_approval_notification') && get_option('w2dc_admin_notifications_email')) {
						...
						if (get_option('w2dc_claim_decline_notification') && get_option('w2dc_admin_notifications_email')) {
							$claimer = get_userdata($this->current_listing->claim->claimer_id);
							...
							$subject = "[" . get_option('blogname') . "] " . __('Claim decline notification', 'W2DC');
	...
	public function saveInitialDraft($post_id) {
		...
		return $wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->w2dc_levels_relationships} (post_id, level_id) VALUES(%d, %d) ON DUPLICATE KEY UPDATE level_id=%d", $this->current_listing->post->ID, $this->current_listing->level->id, $this->current_listing->level->id));
	}
