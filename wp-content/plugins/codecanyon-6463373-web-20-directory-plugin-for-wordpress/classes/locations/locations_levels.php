...
class w2dc_locations_levels {
	...
	public function createLevelFromArray($array) {
		...
		if ($wpdb->insert($wpdb->w2dc_locations_levels, $insert_update_args)) {
			...
			do_action('w2dc_update_locations_level', $new_level_id, $this, $insert_update_args);
	...
	public function saveLevelFromArray($level_id, $array) {
		...
		if ($wpdb->update($wpdb->w2dc_locations_levels, $insert_update_args,	array('id' => $level_id), null, array('%d')) !== false) {
			do_action('w2dc_update_locations_level', $level_id, $this, $insert_update_args);
	...
	public function deleteLevel($level_id) {
		...
		$wpdb->delete($wpdb->w2dc_locations_levels, array('id' => $level_id));
		return true;
	}
}
...
add_action('w2dc_update_locations_level', 'w2dc_update_locations_level', 10, 3);
function w2dc_update_locations_level($level_id, $locations_level, $array) {
