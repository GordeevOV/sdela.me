...
class w2dc_search_widget extends WP_Widget {
	...
	public function widget($args, $instance) {
		...
		if ((!$instance['visibility'] || !empty($w2dc_instance->frontend_controllers)) && (get_option('w2dc_show_what_search') || get_option('w2dc_show_where_search'))) {
			...
			w2dc_renderTemplate('widgets/search_widget.tpl.php', array('args' => $args, 'title' => $title, 'uid' => $instance['uid']));
		}
	}

	public function form($instance) {
		$defaults = array('title' => __('Search listings', 'W2DC'), 'uid' => '', 'visibility' => 1, 'search_visibility' => 1);
...
class w2dc_categories_widget extends WP_Widget {
	...
	public function widget($args, $instance) {
		...
		if (!$instance['visibility'] || !empty($w2dc_instance->frontend_controllers)) {
			...
			w2dc_renderTemplate('widgets/categories_widget.tpl.php', array('args' => $args, 'title' => $title, 'depth' => $instance['depth'], 'counter' => $instance['counter'], 'subcats' => $instance['subcats'], 'parent' => $parent));
		}
	}
	
	public function form($instance) {
		$defaults = array('title' => __('Categories list', 'W2DC'), 'depth' => 1, 'counter' => 0, 'subcats' => 0, 'related_subcats' => 0, 'visibility' => 1, 'parent' => 0);
		$instance = wp_parse_args((array) $instance, $defaults);
...
class w2dc_locations_widget extends WP_Widget {
	...
	public function widget($args, $instance) {
		...
		if (!$instance['visibility'] || !empty($w2dc_instance->frontend_controllers)) {
			...
			if ($instance['related_sublocations']
			&& ($directory_controller = $w2dc_instance->getShortcodeProperty('webdirectory'))
			...
			w2dc_renderTemplate('widgets/locations_widget.tpl.php', array('args' => $args, 'title' => $title, 'depth' => $instance['depth'], 'counter' => $instance['counter'], 'sublocations' => $instance['sublocations'], 'parent' => $parent));
		}
	}
	
	public function form($instance) {
		$defaults = array('title' => __('Locations list', 'W2DC'), 'depth' => 1, 'counter' => 0, 'sublocations' => 0, 'related_sublocations' => 0, 'visibility' => 1, 'parent' => 0);
		$instance = wp_parse_args((array) $instance, $defaults);
